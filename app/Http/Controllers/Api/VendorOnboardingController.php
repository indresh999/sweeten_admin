...
class VendorOnboardingController extends Controller
{
    ...
    public function sendSignupOtp(Request $request): JsonResponse
    {
        ...
        try {
            ...
            Log::error('[Onboarding] OTP mail failed for ' . $email . ': ' . $e->getMessage());
    ...
    public function uploadPhotos(Request $request): JsonResponse
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'tag'      => 'nullable|in:logo,cover,gallery',
        ]);
        ...
        foreach ($request->file('photos') as $file) {
            ...
            ProcessShopPhoto::dispatch($photo->id);

            $created[] = $this->formatPhoto($photo);
        }
    ...
    {
        $photo = ShopImage::where('shop_id', $request->user()->shop_id)->findOrFail($id);
    ...
    public function submitForReview(Request $request): JsonResponse
    {
        ...
        if ($shop->status !== 'draft') {
            return response()->json(['status' => false, 'message' => 'Only draft shops can submit for review.'], 403);
        }

        // Validate minimum required details are filled
        $missing = [];
        foreach (['full_name', 'phone_number', 'restaurant_name', 'restaurant_address', 'city', 'state', 'zip_code'] as $field) {
