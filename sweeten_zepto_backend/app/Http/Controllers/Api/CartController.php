...
{
    public function listCart(Request $request, int $userId): JsonResponse
    {
        ...
        $deliveryCharge = $subtotal >= $freeAbove ? 0 : (float) AppSetting::get('delivery_base_charge', 30);
        ...
        return response()->json([
            ...
            'data'   => [
                ...
                'total'          => round($subtotal + $gst + $deliveryCharge, 2),
            ],
        ]);
    }
    ...
    private function formatCartItem(CartItem $c): array
    {
        ...
        if ($item && $item->images) {
            ...
            $imageUrls = collect($images ?? [])->map(fn($p) => asset('storage/' . $p))->toArray();
        }
    ...
    public function addToCart(Request $request): JsonResponse
    {
        ...
        $existingShop = CartItem::where('user_id', $request->user_id)->value('owner_id');
        if ($existingShop && $existingShop != $request->owner_id) {
            return response()->json([
                'status'  => false,
                'message' => 'Your cart has items from another restaurant. Clear cart to add from this one.',
                'code'    => 'DIFFERENT_SHOP',
            ], 409);
        }
    ...
    public function applyCoupon(Request $request): JsonResponse
    {
        ...
        if ((float)$request->order_amount < (float)$coupon->min_order_amount) {
            return response()->json(['status' => false, 'message' => "Minimum order ₹{$coupon->min_order_amount} required for this coupon"], 422);
        }
        ...
        return response()->json([
            ...
            'coupon'   => ['id' => $coupon->id, 'code' => $coupon->code, 'title' => $coupon->title, 'discount_type' => $coupon->discount_type, 'discount_value' => $coupon->discount_value],
            'discount' => $discount,
        ]);
    }
    ...
    public function availableCoupons(Request $request): JsonResponse
    {
        ...
        $coupons = Coupon::where('is_active', 1)
            ...
            ->map(function ($c) use ($request) {
                ...
                $c->is_applicable = $used < $c->usage_per_user;
